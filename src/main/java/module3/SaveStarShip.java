...
public class SaveStarShip {
    ...
    public int calculateFuelPrice(String fuel, int count) {
        ...
        int price = 50;
        if (fuel.equalsIgnoreCase("STAR100")) {
            price = 70;
        } else if (fuel.equalsIgnoreCase("STAR500")) {
            price = 120;
        } else if (fuel.equalsIgnoreCase("STAR1000")) {
            price = 200;
        }
        return count * price;
    }
    ...
    public boolean isHangarOk(int side1, int side2, int price) {
        ...
        boolean squareHangar = (side1 * side2 >= 1500);
        boolean lengthHangar = ((side1 <= side2 * 2) || (side2 <= side1 * 2));
        boolean priceHangar = ((price / (side1 * side2)) <= 1000);

        return squareHangar & lengthHangar & priceHangar;
    }
